...
menuButton.addEventListener('click', () => {
	...
	favJokes.style.transform = 'translateX(0px)'
	filter.style.opacity = '1'
	filter.style.display = 'block'
	
	if(clickCount % 2 === 0){
		menuImg.src = 'visual/menu-open.svg'
		favJokes.style.transform = 'translateX(100%)'
...
let createCategories = (res) => {
	...
	res.forEach((el) => {
		...
		radio.setAttribute('class', 'categories')
...
let render = (data) => {
	...
	data.forEach(el => {
		...
		jokeCategory.classList.add('joke-category-liked')
		...
		categoryText.textContent = el.categories.toString().toUpperCase()
		...
		jokeCategory.appendChild(categoryText)
...
form.addEventListener('submit', (event) => {
	...
	let radio = document.querySelector('.categories')
	if(radio.checked){
		getJokeByCategory(radio.value)
		.then(res => res.json())
		.then((res => render([res])))

	} else if (randomJoke.checked){
		getRandomJoke()
		.then((res => res.json()))
		.then((res => render([res])))
	}
